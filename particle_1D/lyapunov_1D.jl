...
function phi_mat(z, zstar, alpha, lambda, tau; tol=0.)
    ...
    for I=1:mstar
        for i=1:m
            disti = torus_dist_1D(zstar[I], z[i])
    ...
    if tol == 0.
        dzstar_min = minimum([torus_dist_1D(zstar[I], zstar[J]) for I=1:mstar for J=1:I-1])
...
function agg_pos_tracecov(w, z, wstar, zstar; alpha, lambda, tau, phi_mat_z=nothing)
    ...
    for I=1:mstar
        for i=1:m 
            out[I] .+= phi_mat_z[I,i] * w[i] * (z[i] - z_agg[I])^2
        end
        out[I] /= w_agg[I]
    end
    return out
...
function V_pos(w, z, wstar, zstar, eta_z;
    ...
    s = 0
    for I in eachindex(zstar)
        for i in eachindex(z)
            disti = torus_dist_1D(zstar[I], z[i])
            s += w[i] * phi_mat_z[I,i] * disti^2 / 2
        end
    end
    return s / eta_z
end
