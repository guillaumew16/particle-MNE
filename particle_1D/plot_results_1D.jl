...
function plot_V(gfun, T, copies_w, copies_z, avg_w, avg_z,
    ...
    eps = 1e-30 # avoid PyPlot crashing when y-axis=0 in log plot
    # plt_V_log = plot(range(1, stop=T, step=evalVevery), eps.+ max.(0, Vtots[1:evalVevery:T]), xlabel="k", label="total", yscale=:log10)
    ...
    plt_V_log = plot(range(1, stop=T, step=evalVevery), eps.+ max.(0, Vtots[1:evalVevery:T]), xlabel="k", label="total", linewidth=3, linealpha=0.75, yscale=:log10)
    plot!(range(1, stop=T, step=evalVevery), eps.+ max.(0, Vweis[1:evalVevery:T]), xlabel="k", line=:dot, label="weight term", linewidth=2, yscale=:log10)
    plot!(range(1, stop=T, step=evalVevery), eps.+ max.(0, Vposs[1:evalVevery:T]), xlabel="k", line=:dash, label="position term", linewidth=1, yscale=:log10)
...
function plot_V_xy(gfun, T, copies_wx, copies_x, copies_wy, copies_y, avg_wx, avg_x, avg_wy, avg_y,
    ...
    eps = 1e-30 # avoid PyPlot crashing when y-axis=0 in log plot
    # plt_Vxy_log = plot(range(1, stop=T, step=evalVevery), eps.+ max.(0, Vtots_xy[1:evalVevery:T]), xlabel="k", label="total", yscale=:log10)
    ...
    plt_Vxy_log = plot(range(1, stop=T, step=evalVevery), eps.+ max.(0, Vtots_xy[1:evalVevery:T]), xlabel="k", label="total", linewidth=3, linealpha=0.75, yscale=:log10)
    plot!(range(1, stop=T, step=evalVevery), eps.+ max.(0, Vweis_xy[1:evalVevery:T]), xlabel="k", line=:dot, label="weight term", linewidth=2, yscale=:log10)
    plot!(range(1, stop=T, step=evalVevery), eps.+ max.(0, Vposs_xy[1:evalVevery:T]), xlabel="k", line=:dash, label="position term", linewidth=1, yscale=:log10)
    # hline!([div0s[T+2]], label="avg iterate") --> not defined
    if !hidetitle
